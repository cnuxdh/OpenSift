...
/************************* Local Function Prototypes *************************/
 ...
 struct kd_node* kd_node_init( struct feature*, int n, int newMalloc );
...
struct kd_node* kdtree_build( struct feature* features, int n )
{
  ...
  kd_root = kd_node_init( features, n , 1);
...
void kdtree_release( struct kd_node* kd_root )
{
  ...
  /*
  if(kd_root->features != NULL)
  {
	  free(kd_root->features);
	  kd_root->features = NULL;
  }
  */
...
*/
struct kd_node* kd_node_init( struct feature* features, int n, int newMalloc )
{
  int i;
  ...
  if(newMalloc>0)
  {  
	  //revised by xiedonghai, for multi-thread  
	  kd_node->features = (struct feature*)malloc( n*sizeof(struct feature));
	  for(i=0; i<n; i++)
	  {
		  //printf(".");
		  kd_node->features[i].index = features[i].index;
		  kd_node->features[i].d = features[i].d;
		  memcpy(kd_node->features[i].descr, 
				 features[i].descr, 
				 features[i].d*sizeof(double));
	  }
  }
  else
  {
...
static void partition_features( struct kd_node* kd_node )
{
  ...
  kd_node->kd_left = kd_node_init( features, j + 1, 0 );
  kd_node->kd_right = kd_node_init( features + ( j + 1 ), ( n - j - 1 ), 0);
}
